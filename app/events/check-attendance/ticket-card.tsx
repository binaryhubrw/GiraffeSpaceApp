...
export default function TicketCard({
  ...
  function openInvitationEdit() {
    ...
    if (invitationData.address) {
      if (typeof invitationData.address === "string") {
        ...
        invitationData.address.province &&
        invitationData.address.district &&
        invitationData.address.sector
      ) {
        addressString = `${invitationData.address.province}, ${
          invitationData.address.district
        }, ${invitationData.address.sector}${
          invitationData.address.country
            ? `, ${invitationData.address.country}`
            : ""
        }`;
      }
    ...
    if (invitationData) {
      ...
      try {
        ...
        const ticketCode =
          scannedTicketCode ||
          storedTicketCode ||
          invitationData.freeRegistrationId;
        ...
        if (response.success) {
          ...
          toast.success(
            response.message || "Attendance confirmed successfully!"
          );
        } else {
          setAttendanceResult({
            success: false,
  ...
  async function handleSaveInvitationEdit() {
    ...
    if (!trimmedName || !trimmedPhone || !trimmedGender) {
      toast.error(
        "Missing information. Name, phone number, and gender are required."
      );
      return;
    }
    ...
    try {
      ...
      // Import ApiService dynamically to avoid circular dependencies
      const ApiService = (await import("@/api/apiConfig")).default;
      const response = await ApiService.updateInvitationDetails(
        invitationData.freeRegistrationId,
        requestBody
      );
      ...
      const errorMessage =
        error.response?.data?.message ||
        error.message ||
        "Failed to update invitation details. Please try again.";
      toast.error(errorMessage);
    }
  }
    ...
    return (
      <Card className="border-2 border-green-200 bg-green-50">
          ...
          <div className="bg-white p-4 rounded-lg border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div className="flex items-center gap-2 text-gray-700">
                ...
                <span className="font-medium text-black">
                  {invitationData.event.eventName}
                </span>
              </div>
              <div className="flex items-center gap-2 text-gray-700">
                <Calendar className="h-4 w-4" />
                <span className="font-medium text-black">
                  {invitationData.event.bookingDates[0]
                    ? new Date(
                        invitationData.event.bookingDates[0].date
                      ).toLocaleDateString()
                    : "Not specified"}
                </span>
              </div>
              <div className="flex items-center gap-2 text-gray-700">
                <Calendar className="h-4 w-4" />
                <span className="font-medium text-black">
                  {invitationData.event.startTime} -{" "}
                  {invitationData.event.endTime}
                </span>
              </div>
              <div className="flex items-center gap-2 text-gray-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div className="flex items-center gap-2 text-gray-700">
                ...
                <span>
                  Registered by:{" "}
                  <span className="font-medium text-black">
                    {invitationData.registeredByDetails.firstName}{" "}
                    {invitationData.registeredByDetails.lastName}
                  </span>
                </span>
              </div>
              <div className="flex items-center gap-2 text-gray-700">
                ...
                <span>
                  Email:{" "}
                  <span className="font-medium text-black">
                    {invitationData.registeredByDetails.email}
                  </span>
                </span>
              </div>
              <div className="flex items-center gap-2 text-gray-700">
                ...
                <span>
                  ...
                  <span className="font-mono text-black">
                    {invitationData.registeredByDetails.phoneNumber}
                  </span>
                </span>
              </div>
              <div className="flex items-center gap-2 text-gray-700">
        ...
        <Dialog
          ...
          onOpenChange={(open) => !open && closeInvitationEdit()}
        >
          <DialogContent className="sm:max-w-md bg-white text-black">
            <DialogHeader>
              <DialogTitle>Edit Attendee Information</DialogTitle>
              <DialogDescription className="text-gray-600">
                Update attendee name, phone number, gender, and address
                information.
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="flex gap-2">
              ...
              <Button
                onClick={handleSaveInvitationEdit}
                className="bg-blue-600 hover:bg-blue-700 text-white"
              >
                Save changes
              </Button>
